...
1. `RuntimeServices` 完成运行时依赖装配，`AgentRuntime` 只保留运行流程和少量对外方法。
2. `AgentRuntime.ask()` 接收用户输入和 session。
3. `SessionManager` 准备 SDK `SQLiteSession`，必要时压缩历史上下文。
4. `runtime.py` 构造 Orchestrator Agent，并按普通/流式模式调用 OpenAI Agents SDK Runner。
5. `tool_factory` 按 bot 配置挂载 runtime tools 和 subagent tools。
6. `RuntimeContext` 和 `EventBus` 记录运行事件。
7. subagent extension 注册 tools、prompt context、capability resolver 和 ResultFormatter。
8. `ArtifactStore` 按标准 artifact envelope 保存大结果，`result_mapper` 将最终输出和事件流整理成 API 返回结构。
...
| `runtime.py`         | 主门面。`ask()` 是一次用户请求的主流程入口，负责 build context、prepare session、build agent、run、map result。 | 不代理 services 字段；调用方必须显式访问 `runtime.services.*`。 |
| `services.py`        | 运行时依赖容器。集中创建 registry、bot/resource loader、memory、artifact store、formatter、subagent runner、session manager、hooks。 | 依赖装配不再散落在 `AgentRuntime.__init__`。 |
...
| `context.py`         | 定义 `RuntimeContext`，承载一次 run 的共享上下文：session、事件流、artifact store、registry、父子 subagent 关系和 namespaced typed state。            | typed state 只提供 run-scoped 容器和 namespace 隔离，不解释 Text2SQL/RAG 等业务语义；child run 不继承父 run 的 typed state。                                                                                                                                                                                                                                                                |
| `events.py`          | 定义事件类型 `EventKind`、事件对象 `RuntimeEvent` 和 `EventBus`，统一 runtime 内部事件格式、顺序号和 callback 通知。           | 事件层只维护标准 wire shape 和 callback 分发，不承载业务路由语义。                                                                                                                             |
...
| `model_profiles.py`  | 从 `CHAT_*` 环境变量解析单一 chat client profile，并规范化输出 token 上限。                              | 已去掉 orchestrator/executor 模型角色；embedding 配置由 memory embedding 模块独立保留。subagent 可在 `call_model()` 时只覆盖模型名，仍复用同一 base URL/key。                                                                                                                    |
...
| `result_formatters.py` | 定义 `ResultArtifactSpec`、`ResultFormatter` 协议和 registry。core 只拥有通用 artifact 与 memory formatter；subagent formatter 由各自 extension 注册。 | 已落地 `framework owns the contract, subagent owns the mapping`。formatter 返回完整标准 artifact summary 所需的 spec，支持 rows 之外的通用 `preview` envelope。 |
| `result_events.py`   | 从标准 `result_created.result` envelope 中提取 ArtifactStore metadata。                            | 业务结果解析不再集中硬编码在这里；非标准旧 wire shape 不再解析。 |
...
### ResultFormatter 与 ArtifactStore
...
- `ArtifactStore` 当前按 artifact-neutral schema 保存：`result_artifacts` + `result_artifact_rows`，并在 artifact metadata 外记录 `run_id`、`session_id`、`bot_id` 作为读取 scope。
- `preview.kind == "rows"` 是默认表格预览；后续 chart/file/report 可以通过同一个 `preview` envelope 暴露非表格摘要。
...
- 已移除 `agent_runtime/core/orchestrator.py` 旧入口，内部引用统一改为 `agent_runtime.core.runtime`。
- 已移除 `agent_factory.py`、`run_executor.py`、`tool_protocol.py` 三个轻量包装层。
- 已移除模型角色体系，runtime 只保留单一 chat client profile 和独立 embedding profile；Text2SQL SQL generation 只覆盖模型名，不新建独立 URL/key 配置。
- 已移除 Orchestrator/Text2SQL 的 `get_current_time` 工具，当前时间改为 prompt 上下文注入。
- 已落地 `ResultFormatter` 注册化：Text2SQL/RAG 在 extension 中注册 formatter，core/memory 注册 memory formatter，ArtifactStore 改为 artifact-first。
...
- 已落地 subagent tool policy metadata：普通工具自动获得默认 capability/audit metadata，高风险工具可显式绑定 capability 和 policy，工具事件统一带审计字段。
- 已收窄 `SubagentContext` 公共 API，只向 extension 暴露运行必需能力，不再暴露裸 cache、profile、SQL 专属存储入口或手动 result event。
- 已落地 ArtifactStore scope 字段和可选读取校验，服务层分页/导出接口可传入 run/session/bot scope。
- 标准 artifact summary 由 formatter/spec 直接决定，core 不再做 SQL/RAG 定制化推断。
- ArtifactStore 旧 SQL 专用表直接删除并使用 artifact-neutral schema，本轮不保留测试数据迁移。
- SQL generation 性能实验暂时回退，架构治理优先于延迟优化。
- 已抽出 `RuntimeServices`，`AgentRuntime` 不再代理 services 字段，`ask()` 主链路更轻。
- 已将 manifest model、parser、AgentRegistry 从旧 `skill_registry.py` 大文件拆出；`skill_registry.py` 只导出 `SkillRegistry`。
- `SubagentContext` 新增只读 artifact metadata/page 读取入口，供后续 data_analysis/chart worker 使用。
