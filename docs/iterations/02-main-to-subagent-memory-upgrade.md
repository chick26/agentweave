...
早期版本的 Text2SQL 更接近“纯工具调用”形态：主编排模型直接围绕一组本地工具完成 domain/schema/value/SQL/execute 流程。这个方式能跑通结构化数据查询，但数据库问答逻辑和主会话上下文耦合较紧：
...
| 结果返回 | 容易把数据样例带回模型上下文 | Result Store 持久化上限内结果，只返回 pointer + sample |
...
sequenceDiagram
    ...
    DB-->>W: 查询结果
    W->>RS: 上限内结果落库
    W-->>O: answer + result_id + sample_rows + has_more + trace
    O-->>User: 简洁中文结论
```
...
当前 prompt 要求 Orchestrator 只传递用户原文和已确认事实，不替 subagent 推断 schema 字段、枚举值或 SQL 条件。这样可以避免主模型在不了解 domain schema 的情况下写出 schema-like task。
...
`DOMAIN.md` 可以声明业务口径，例如某个指标对应哪些字段和值。但代码不对子串做硬匹配，也不直接把某个 metric 注入为过滤条件。这样能避免否定语义和反向条件被简单子串匹配误伤。
