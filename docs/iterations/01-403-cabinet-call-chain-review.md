...
> 当前状态：本文最初用于审查一次具体调用链。后续重构已经删除 `infer_metric_intent`、`infer_limit`、`infer_display_intent` 等 Python 推断层；`SQLPlan` 只保留事实字段，`business_metrics` 作为 domain 上下文交给 SQL 模型判断是否适用。
...
sequenceDiagram
    ...
    Planner->>SQLModel: schema + SQLPlan + 用户问题
...
- `app.py:984`：读取用户输入。
- `app.py:1033`：调用 `runtime.ask(prompt, session_id, event_callback, max_turns)`。
- `app.py:1041`：收集最终输出、model logs 和 events。
- `app.py:1089`：持久化诊断日志。

### 2. Orchestrator 运行入口

`AgentRuntime.ask()` 位于 `agent_runtime/orchestrator.py`：

- `orchestrator.py:146`：打开当前会话的 `SQLiteSession`。
- `orchestrator.py:148`：对历史消息做上下文压缩判断。
- `orchestrator.py:159`：创建 Orchestrator Agent。
- `orchestrator.py:161`：调用 `_build_instructions(session_id, current_query=user_input, context=context)`。
- `orchestrator.py:173`：构造 Orchestrator 可见工具。
- `orchestrator.py:176`：调用 `Runner.run(...)` 执行模型循环。

### 3. Orchestrator Prompt 组装

主 Prompt 从 `agent_runtime/prompts.py` 的 `SYSTEM_PROMPT` 开始，再追加运行时上下文。核心组装位置：

- `orchestrator.py:209`：以 `SYSTEM_PROMPT` 作为基础。
- `orchestrator.py:217`：可选读取用户记忆文件。
- `orchestrator.py:218`：读取 `PROJECT.md`。
- `orchestrator.py:224`：构造 query-aware memory context。
- `orchestrator.py:240`：将 `<memory_context>` 注入 instructions。
...
Orchestrator 的工具面在 `agent_runtime/orchestrator.py:473` 附近构造，典型包括：
...
Text2SQL Worker 由 `agent_runtime/skill_runner.py` 启动：

- `skill_runner.py:114`：读取 `text2sql` manifest。
- `skill_runner.py:123`：解析 worker 使用的 model profile。
- `skill_runner.py:149`：创建隔离的 `RunContext`。
- `skill_runner.py:167`：构造 worker agent，传入 `prompt_query=task`。
- `skill_runner.py:181`：使用内存 `SQLiteSession` 执行 `Runner.run(...)`。

Worker 与 Orchestrator 使用隔离的 session，因此 Text2SQL 内部的试错、SQL 报错、schema 细节不会污染主会话。

### 2. Worker Prompt 来源

Worker Prompt 由 `skill_runner.py:348` 附近的 `_build_worker_prompt()` 组装，主要来源：
...
`plan_sql_query` 位于 `subagents/text2sql/tools.py:64`，对模型表现为一个高层工具，对代码内部则完成以下工作：
...
SQL generation prompt 位于 `subagents/text2sql/prompts.py`，其中也要求：
...
SQL 生成位于 `subagents/text2sql/tools.py:270` 附近。SQL 模型收到的信息包括：
...
执行阶段位于 `subagents/text2sql/tools.py:153`：

- 执行只读 SQL。
- 将完整结果写入 Result Store。
- 只返回 `result_id`、`row_count`、`sample_rows`、`truncated`。
