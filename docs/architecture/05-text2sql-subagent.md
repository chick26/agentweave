...
```mermaid
sequenceDiagram
    Orchestrator->>SubagentRunner: text2sql(task="查询某类业务指标")
...
不同的数据表属于不同的业务域。`Text2SQLDomainRegistry` 会扫描 `domains/*/DOMAIN.md`，把相关的字段说明和业务逻辑打包装进 Worker 的 prompt 中。公开仓库中的 domain 只是接入模式示例，真实业务 domain 和数据文件应在本地配置。

### plan_sql_query：装载事实并生成 SQL

Worker 并不直接手写 SQL，而是先调用 `plan_sql_query`。这一步非常重：
1. 激活 Domain，加载表 Schema、字段说明和 `business_metrics`。
2. **值链接（Value Linking）**：比如用户输入的是自然语言实体名，代码会先去真实数据库 `search_values()` 查找可用候选值。这极大减少了 SQL 中因为名字不一致造成的空结果。
...
`business_metrics` 来自 `DOMAIN.md`，作为 domain 事实上下文传给 SQL 模型。模型可以据此判断业务口径是否适用，但代码不会用子串匹配强行注入过滤条件。
