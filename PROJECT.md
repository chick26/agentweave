...
Core boundaries:
- `subagents/` contains delegated worker agents with `AGENT.yaml + prompt.md + extension.py`.
- `skills/` contains loadable method cards with `SKILL.md`; skills are not worker agents.
- The orchestrator chooses top-level subagents and summarizes their `answer` plus typed artifacts.
