...
Composable agent runtime built with Streamlit + OpenAI Agents SDK. The main Orchestrator handles intent routing and subagent delegation; Text2SQL is the first specialized subagent and runs in an isolated worker context. IDC、海缆等建模为 Text2SQL subagent 内部的数据 domain/resource，不再称为 skill。
...
sequenceDiagram
    ...
    User->>Orchestrator: 提出数据提问 (如 "403机房有多少可用机柜？")
...
3. **沙箱隔离执行阶段（Subagent Execution）**：
   ...
   * **值链接**：对于用户输入中拼写不精确的实体（如 "403机房"），`plan_sql_query` 会在内部查询数据库真实值候选（如 "Room-403"），帮助 SQL 模型生成更 grounded 的 SQL。
...
| Text2SQL domain | `idc_resources` | IDC 机房/机柜/数据中心资源 |
| Text2SQL domain | `sea_cable_faults` | 海缆故障记录、维修状态、影响城市等 |
