...
sequenceDiagram
    ...
    W->>A: Persist full artifacts when needed
...
| ArtifactStore | SQLite-backed storage for large outputs; workers expose typed artifacts with pointers, row counts, and previews. |
...
| Text2SQL SQL generation model | `TEXT2SQL_SQL_MODEL` | `qwen3-32b`, using `CHAT_BASE_URL` / `CHAT_API_KEY` |
...
env files, prepared SQLite/RAG indexes, memory, artifact store, and Streamlit
...
2. Add `AGENT.yaml` with `name`, `description`, `execution.mode: worker`, `extension`, and `routing_hints`. Add `capabilities`, `policies`, `output_contract`, `memory`, or domain-specific fields only when the subagent needs explicit governance or typed artifacts. Worker and extension chat calls reuse the runtime `CHAT_*` client profile unless a subagent explicitly overrides only the model name.
3. Add `prompt.md` for the worker prompt.
4. Implement `extension.py` with `register(api)` to register tools, result formatters, capability resolvers, environment checks, and prompt context. Plain tools may use `api.tool(fn)`; high-risk tools should pass explicit `capability`, `policy_path`, and `audit_name`.
...
| `capabilities` | No | Explicit auditable capability package names for governed tools, for example `db.readonly` or `rag.search`. Plain tools get default `<subagent>.tool.<tool>` capability metadata. |
| `policies` | No | Capability constraints such as row limits, timeout, allowed schemas, and recursive-call policy. Governed tools bind to these with `policy_path`; tools must still enforce security. |
...
Model role fields are removed; worker chat calls reuse the runtime `CHAT_*`
client profile. Text2SQL SQL generation overrides only the model name through
`TEXT2SQL_SQL_MODEL`.
